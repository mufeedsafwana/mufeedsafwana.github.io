  ...
  canvases.forEach((canvas) => {
    ...
    function checkDone() {
      ...
      if (percent > 45 && !isCompleted) {
        ...
        completedCount++;
        if (
          completedCount === canvases.length &&
...
function setupCountdown() {
  const target = new Date("September 10, 2026 12:30:00").getTime();
  const els = {
...
const CALENDAR_EVENTS = {
  nikah: {
    uid: "nikah-mufeed-safwana@Mufeed-safwana.vercel.app",
...
function downloadICS(event) {
  ...
  const ics = [
    "BEGIN:VCALENDAR",
    "VERSION:2.0",
    "PRODID:-//Mufeed Safwana Wedding//EN",
    "CALSCALE:GREGORIAN",
    "METHOD:PUBLISH",
    "BEGIN:VEVENT",
    `UID:${event.uid}`,
...
function setupRSVP() {
  ...
  yesBtn.addEventListener("click", (e) => {
    message.textContent =
      "Alhamdulillah! We cannot wait to celebrate with you!";
  ...
  noBtn.addEventListener("click", () => {
    message.textContent =
      "We will keep you in our duas. Jazakallahu Khair for letting us know.";
    message.style.opacity = "1";
